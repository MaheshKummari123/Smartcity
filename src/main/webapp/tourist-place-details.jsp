<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="com.smartcity.model.TouristPlace,java.util.List"%>

<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>SmartCity | Tourist Place Details</title>
<link
	href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css"
	rel="stylesheet">

<style>
.rating {
	color: #ffc107;
}

.hero-img {
	height: 400px;
	object-fit: cover;
}

.section-box {
	background: #f8f9fa;
	padding: 20px;
	border-radius: 8px;
}
.app-icon {
	            width: 30px;
	            height: 30px;
	            border-radius: 5px;
	            background: linear-gradient(135deg, #0a2a52, #0d3a6e);
	            border: 1.5px solid rgba(0, 200, 255, 0.3);
	            display: flex;
	            align-items: center;
	            justify-content: center;
	            margin-right: 10px;
	            box-shadow: 0 4px 20px rgba(0, 150, 255, 0.25), 0 0 30px rgba(0, 200, 255, 0.1);
	        }
</style>
</head>
<body>

	<!-- Navbar -->
	<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
		<div class="container">
			<div class="container d-flex align-itmes-center">
			<div class="d-flex align-items-center">
				<div class="app-icon">
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <!-- City building -->
                    <rect x="6" y="18" width="8" height="14" fill="#0e6ea8" rx="1" />
                    <rect x="8" y="20" width="2" height="2" fill="#00c8ff" opacity="0.7" />
                    <rect x="12" y="20" width="2" height="2" fill="#00c8ff" opacity="0.7" />
                    <rect x="8" y="24" width="2" height="2" fill="#00c8ff" opacity="0.5" />
                    <!-- Tall center building -->
                    <rect x="16" y="10" width="10" height="22" fill="#0a5080" rx="1" />
                    <rect x="20" y="7" width="2" height="5" fill="#00e5b0" opacity="0.7" />
                    <rect x="18" y="13" width="2" height="2" fill="#00c8ff" opacity="0.7" />
                    <rect x="22" y="13" width="2" height="2" fill="#00c8ff" opacity="0.7" />
                    <rect x="18" y="18" width="2" height="2" fill="#00c8ff" opacity="0.5" />
                    <rect x="22" y="18" width="2" height="2" fill="#00c8ff" opacity="0.5" />
                    <!-- Right building -->
                    <rect x="28" y="16" width="7" height="16" fill="#0e6ea8" rx="1" />
                    <rect x="29" y="19" width="2" height="2" fill="#00c8ff" opacity="0.7" />
                    <rect x="33" y="19" width="2" height="2" fill="#00c8ff" opacity="0.5" />
                    <!-- Ground line -->
                    <line x1="4" y1="32" x2="34" y2="32" stroke="#00c8ff" stroke-width="1.2" stroke-opacity="0.5" />
                    <!-- Signal arc -->
                    <path d="M21 4 Q28 4 28 11" stroke="#00e5b0" stroke-width="1.2" fill="none" opacity="0.6"
                        stroke-linecap="round" />
                    <path d="M21 2 Q31 2 31 11" stroke="#00e5b0" stroke-width="0.8" fill="none" opacity="0.35"
                        stroke-linecap="round" />
                </svg>
            </div>
			
			<a class="navbar-brand fw-bold" href="#">Smart City</a>
			</div>
			<h2 class="navbar-brand">Tourist Place Details</h2>
		</div>
	</nav>

	<div class="container my-5">
		<%String source=(String)session.getAttribute("source"); %>
		<!-- Back Button -->
		<a href="<%=source %>" class="btn btn-secondary mb-3">← Back
			to Attractions</a>

		<!-- Main Card -->
		<div class="card shadow">
			<%TouristPlace tourist=(TouristPlace)session.getAttribute("touristById"); 
			if(tourist!=null){
			%>
			<!-- Large Image -->
			<img src="<%=tourist.getImageUrl() %>"
				class="card-img-top hero-img">

			<div class="card-body">

				<!-- Name & Category -->
				<div class="d-flex justify-content-between align-items-center">
					<h2 class="fw-bold"><%=tourist.getName() %></h2>
					<span class="badge bg-primary"><%=tourist.getCategory() %></span>
				</div>

				<!-- Rating -->
				<p class="rating fs-5 mt-2">⭐ <%=tourist.getRating() %> / 5</p>

				<hr>

				<!-- Description -->
				<h5>About This Place</h5>
				<p><%=tourist.getDescription() %></p>

				<div class="row mt-4">

					<!-- Left Info -->
					<div class="col-md-6">
						<div class="section-box mb-3">
							<h6>📍 Address</h6>
							<p><%=tourist.getAddress() %></p>
						</div>

						<div class="section-box mb-3">
							<h6>🕒 Opening Hours</h6>
							<p><%=tourist.getOpeningHours() %></p>
						</div>
						
						
						<div class="section-box mb-3">
						<%if(tourist.getCategory().equals("PG")||tourist.getCategory().equals("Hostel")){%>
							<h6>🎟 Monthly Rent</h6>
						
						<% }else { %>
						<h6>🎟 Entry Fee</h6>
						<%} %>
						
							<p>₹<%=tourist.getEntryFee() %></p>
						</div>
					</div>

					<!-- Right Map -->
					<div class="col-md-6">
						<div class="section-box">
							<h6>📌 Location Map</h6>
							<iframe
								src="https://maps.google.com/maps?q=<%=tourist.getLatitude() %>,<%=tourist.getLongitude() %>&z=15&output=embed"
								width="100%" height="250" style="border: 0;"> </iframe>
						</div>
					</div>

				</div>
				
				<!-- Action Buttons -->
				<div class="mt-4 d-flex gap-3">
					<button class="btn btn-primary">❤️ Add to Wishlist</button>
					<button class="btn btn-success">📤 Share</button>
				</div>

			</div>
		</div>
	<%
					}
				%>
	</div>

	<footer class="bg-dark text-white text-center py-3 mt-5"> ©
		2026 SmartCity Tourism Portal </footer>

</body>
</html>
